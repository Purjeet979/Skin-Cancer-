...
for x in models:
    model_path = os.path.join(base_path, "result", f"yolov8{x}", "weights", "best.pt")
    if not os.path.exists(model_path):
        model_path = os.path.join(base_path, "result_with_aug", f"yolov8{x}", "weights", "best.pt")
    if not os.path.exists(model_path):
        model_path = f"yolov8{x}-cls.pt"
